...
//        }
    ...
    public boolean syntacticAnalysis(List<Token> tokenList) {
        ...
        int times = 0;
        int label = 0;
        String[] treeLabel = new String[1000];
        ...
        while ((!stack.isEmpty()) && times < tokenList.size()) {
            ...
            //记录treeLabel中label前最后一个非空元素，即栈顶对应的label
            for (topLabel = label - 1; topLabel >= 0; topLabel--) {
                //System.out.println("topLabel= " + topLabel + "  treeLabel= " + treeLabel[topLabel] + " label= " + label);
                if (treeLabel[topLabel] != null)
                    break;
            }
                        ...
                        for (int j = tempGrammar.length - 1; j >= 0; j--) {
                            if (tempGrammar[j] != null) {
                                if (tempGrammar[j].charAt(0) >= '0' && tempGrammar[j].charAt(0) <= '9') {
                                    ...
                                    treeLabel[label] = numToStr(tempGrammar[j]);
                                } else {
                                    tempGrammarForOut.add(tempGrammar[j]);
                                    treeLabel[label] = tempGrammar[j];
                                }
